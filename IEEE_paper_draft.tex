...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt, topsep=3pt]
  ...
  \item Advanced XAI extensions: SHAP interaction effects, counterfactual
    recourse recommendations for target reduction, and a confidence-vs-
    explanation stability analysis linking conformal interval width with
    LIME consistency.
\end{itemize}
...
using sine/cosine transformations for each cycle.
...
we use mean aggregation for power-like variables and sum aggregation for
...
component with period $m$, and $\varepsilon_t$ the residual.
...
The feature vector at time $t$ combines original covariates, cyclical
time encodings, lagged HW components from~\eqref{eq:hw_lag}, and an
optional autoregressive lag block (e.g., $y_{t-1}, y_{t-24}, y_{t-168}$).
...
with a squared-error loss and tree-complexity regularisation.
...
is applied using training-set statistics only,
...
\textbf{SHAP (TreeSHAP)} computes Shapley-value attributions for each
feature,
...
$\mathbf{x}_0$ by minimizing weighted local approximation error,
with proximity kernel
$\pi_{\mathbf{x}_0}(z) = \exp\!\left(-D(z,\mathbf{x}_0)^2/\sigma^2\right)$~\cite{ribeiro2016lime}.

\textbf{Permutation Feature Importance (PFI)} measures error increase
after shuffling one feature at a time,
where $\mathbf{X}_{\pi_j}$ has column $j$ randomly permuted.

\textbf{Partial Dependence (PDP)} estimates the marginal effect of
feature $j$ by averaging predictions over the empirical distribution of
all other features.

\textbf{SHAP interaction dependence} is used to visualize pairwise
effects by plotting a feature's SHAP value while color-coding by an
interacting feature. This reveals conditional behavior hidden by
single-feature rankings.

\textbf{Counterfactual recourse} addresses the practical question:
``what should change to reduce predicted consumption by 20\%?'' We
search plausible feature ranges and report minimal-change suggestions
that move predictions toward that target.
...
\subsection{Uncertainty and Explanation Stability}

Split conformal prediction intervals are computed at 90\% and 95\%
levels. With calibration residual quantile $\hat{q}_{1-\alpha}$, the
interval for each test point is
$[\hat{y}_{t+1}-\hat{q}_{1-\alpha},\hat{y}_{t+1}+\hat{q}_{1-\alpha}]$.
We report empirical coverage and average width.

To connect uncertainty with explanation reliability, LIME is rerun with
multiple seeds per instance. Stability is measured as mean pairwise
cosine similarity of normalized local weight vectors, then correlated
(Spearman) with 95\% interval width.
...
Adding target lag features at hourly
...
particularly for unusual consumption periods. PFI rankings do not always
match SHAP rankings---a signal of
features whose importance depends on interaction effects rather than
independent marginal contribution. PDP plots
...
SHAP interaction plots reveal that feature influence is context-
dependent. For example, the importance of appliance sub-metering can
increase in specific time windows rather than acting uniformly over the
day.

Counterfactual outputs provide actionable recourse by suggesting
feature changes associated with lower predicted consumption. These are
model-based recommendations (not causal guarantees), but they improve
practical interpretability for end users.

Confidence-vs-stability analysis shows that instances with wider 95\%
intervals tend to have less stable LIME explanations, indicating local
explanations are less reliable when model uncertainty is high.
...
High SHAP values are often shared across
...
Fourth, combining uncertainty and explanation stability is more
informative than either alone. A local explanation attached to a
high-uncertainty prediction should be treated as exploratory rather
than definitive.

\subsection{Threats to Validity}

The main limitations are single-dataset evaluation and limited
hyperparameter search breadth. The dataset covers a single household in France;
...
The main result is not a new state-of-the-art accuracy number. It is a
more honest picture of where a well-built baseline model succeeds and
where it struggles, and a cleaner methodology for connecting feature
importance claims to actual predictive necessity. The added SHAP
interaction, counterfactual, and confidence-vs-stability analyses turn
the interpretability layer into a stronger decision-support workflow.
The daily-scale result ($R^2 = -0.228$) is a useful reminder that
aggregation does not always help: smoothing can collapse the very
signal a model needs.

Future work will focus on: (i)~cross-dataset generalisation to
evaluate robustness across household types and geographies;
(ii)~causal and feasibility-constrained counterfactuals;
(iii)~concept-drift detection and adaptive retraining;
...
and \texttt{pdp\_*} for global explanations; \texttt{shap\_interactions\_*}
for interaction effects; \texttt{counterfactuals\_*} for recourse
recommendations; \texttt{intervals\_*} for uncertainty bands; and
\texttt{lime\_*} plus \texttt{lime\_stability\_*} for local
explanations and stability analysis.
